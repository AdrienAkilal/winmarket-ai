...
if nav == "Analyser un appel d'offres":
    ...
    if st.session_state.current_ao is None:
        from src.ui.capacity_editor import render_capacity_editor
        render_capacity_editor(pipeline)
        st.markdown("---")
        ...
        with col_src:
            mode = st.radio("Source de l'appel d'offres", ["Appels d'offres en stock", "Importer un fichier (PDF/DOCX/TXT)", "Saisie texte libre"], horizontal=True)

        text = None
        if mode == "Appels d'offres en stock":
            if examples:
